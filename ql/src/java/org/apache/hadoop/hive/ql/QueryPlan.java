...
public class QueryPlan implements Serializable {
    ...
    public QueryPlan(
            String queryString, BaseSemanticAnalyzer sem, Long startTime, String queryId,
    ...
    public static String makeQueryId() {
        ...
        return userid + "_" + String.format("%1$4d%2$02d%3$02d%4$02d%5$02d%6$02d", gc.get(Calendar.YEAR),
                gc.get(Calendar.MONTH) + 1, gc.get(Calendar.DAY_OF_MONTH), gc.get(Calendar.HOUR_OF_DAY),
                gc.get(Calendar.MINUTE), gc.get(Calendar.SECOND)) + "_" + UUID.randomUUID().toString();
    }
    ...
    private void populateOperatorGraph(
            org.apache.hadoop.hive.ql.plan.api.Task task, Collection<Operator<? extends OperatorDesc>> topOps) {
        ...
        while (tasksToVisit.size() != 0) {
            ...
            if (task instanceof ExecDriver) {
                ...
                if (mrTask.hasReduce()) {
                    ...
                    Collection<Operator<? extends OperatorDesc>> reducerTopOps = new ArrayList<Operator<?
