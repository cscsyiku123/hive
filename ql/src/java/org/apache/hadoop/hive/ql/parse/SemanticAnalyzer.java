 ...
 * <p>
 * TODO 这个类有点复杂，Hive优化的秘密全在于此
 * TODO 输入的 ASTTree 后续的 QB 的生成，逻辑执行计划、逻辑执行计划的优化、物理执行计划的切分、物理执行计划的优化、
 * TODO 以及 mapreduce 任务的生成全部都在这1万多行的代码里边的逻辑中。
 */
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
    ...
    private final Set<Integer> ignoredTokens = Sets
            .newHashSet(HiveParser.TOK_GROUPBY, HiveParser.TOK_ORDERBY, HiveParser.TOK_WINDOWSPEC, HiveParser.TOK_CLUSTERBY,
    ...
    protected void reset(boolean clearPartsCache) {
        ...
        if(clearPartsCache) {
            ...
            //When init(true) combine with genResolvedParseTree, it will generate Resolved Parse tree from
            // syntax tree
            //ReadEntity created under these conditions should be all relevant to the syntax tree even the ones
            // without parents
    ...
    public ParseContext getParseContext() {
        ...
        return new ParseContext(queryState, opToPartPruner, opToPartList, topOps, new HashSet<JoinOperator>(joinContext.keySet()),
                new HashSet<SMBMapJoinOperator>(smbMapJoinContext.keySet()), loadTableWork, loadFileWork, columnStatsAutoGatherContexts, ctx,
                idToTableNameMap, destTableId, uCtx, listMapJoinOpsNoReducer, prunedPartitions, tabNameToTabObject, opToSamplePruner, globalLimitCtx,
                nameToSplitSample, inputs, rootTasks, opToPartToSkewedPruner, viewAliasToInput, reduceSinkOperatorsAddedByEnforceBucketingSorting,
    ...
    private String processTable(QB qb, ASTNode tabref) throws SemanticException {
        ...
        if(tsampleIndex >= 0) {
            ...
            TableSample tabSample = new TableSample(unescapeIdentifier(sampleClause.getChild(0).getText()),
                    unescapeIdentifier(sampleClause.getChild(1).getText()), sampleCols);
            ...
            if(type.getType() == HiveParser.TOK_PERCENT) {
                ...
                if(percent < 0 || percent > 100) {
                    throw new SemanticException(
                            generateErrorMessage((ASTNode) numerator, "Sampling percentage " + "should" + " be between 0 and 100"));
                }
    ...
    private ASTNode genValuesTempTable(ASTNode originalFrom, QB qb) throws SemanticException {
        ...
        assert virtualTableRef.getToken()
                .getType() == HiveParser.TOK_VIRTUAL_TABREF : "Expected first child of " + "TOK_VIRTUAL_TABLE to be TOK_VIRTUAL_TABREF but was " + virtualTableRef
                .getName();
        ...
        assert valuesTable.getToken()
                .getType() == HiveParser.TOK_VALUES_TABLE : "Expected second child of " + "TOK_VIRTUAL_TABLE to be TOK_VALUE_TABLE but was " + valuesTable
                .getName();
        ...
        try {
            ...
            for(Node n : valuesTableChildren) {
                ...
                assert valuesRow.getToken()
                        .getType() == HiveParser.TOK_VALUE_ROW : "Expected child of " + "TOK_VALUE_TABLE to be TOK_VALUE_ROW but was " + valuesRow
                        .getName();
    ...
    private void assertCombineInputFormat(Tree numerator, String message) throws SemanticException {
        String inputFormat = conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez") ? HiveConf
                .getVar(conf, HiveConf.ConfVars.HIVETEZINPUTFORMAT) : HiveConf.getVar(conf, HiveConf.ConfVars.HIVEINPUTFORMAT);
    ...
    /**
     * // TODO_MA 注释： Hive CTE 是允许用户定义一些 SQL 片段，提高 SQL 的可读性。其实就是 with as 语法，也是子查询的一部分
    ...
    static boolean isJoinToken(ASTNode node) {
        if((node.getToken().getType() == HiveParser.TOK_JOIN) || (node.getToken().getType() == HiveParser.TOK_CROSSJOIN) || isOuterJoinToken(
                node) || (node.getToken().getType() == HiveParser.TOK_LEFTSEMIJOIN) || (node.getToken().getType() == HiveParser.TOK_UNIQUEJOIN)) {
            return true;
        }
    ...
    static private boolean isOuterJoinToken(ASTNode node) {
        return (node.getToken().getType() == HiveParser.TOK_LEFTOUTERJOIN) || (node.getToken().getType() == HiveParser.TOK_RIGHTOUTERJOIN) || (node
                .getToken().getType() == HiveParser.TOK_FULLOUTERJOIN);
    }
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： Phase 1: (including, but not limited to):
     *  -
     ...
     *  -
     *  获取目的地的位置，并将子句命名为“ inclause” + i
     *  2. Gets the location of the destination and names the clause "inclause" + i
     *  -
     *  创建从聚合树的字符串表示形式到实际聚合AST的映射
     ...
     *  -
     *  在aliasToLateralViews中创建从表别名到侧面视图AST的映射
     *  5. Creates a mapping from a table alias to the lateral view AST's in aliasToLateralViews
     *  -
     *  参数 qb 是一个空的 QB，在不同 case 类型下对齐进行填满。
     *  doPhase1 对 ASTTree 中的每个元素的 TOK 类型进行 case，针对于不同的 case 对节点数据进行填充。
     *  for 遍历整棵 ASTTree，中间对每个元素递归调用 doPhase1，这种方式是一种深度优先搜索的算法。
     *  经过一轮深度优先遍历，不带元数据的 QB 树就生成了。
     *  doPhase1 执行完毕之后得到 QB，QB 里边的只是一些关键字还有一些表的名字，但是和 hdfs 的文件路径对应不起来，
     *  所以需要 metaData 映射关系，之后在 SemanticAnalyzer 中调用了 getMetaData
     *
     * @param ast
     * @param qb
     * @param ctx_1
            ...
            switch(ast.getToken().getType()) {
                ...
                case HiveParser.KW_WINDOW:
                    if(!qb.hasWindowingSpec(ctx_1.dest)) {
                        throw new SemanticException(
                                generateErrorMessage(ast, "Query has no Cluster/Distribute " + "By; but " + "has a Window definition"));
                    }
                ...
                case HiveParser.TOK_INSERT:
                    ...
                    if(destination.getChildCount() == 2 && tab.getChildCount() == 2 && destination.getChild(1)
                            .getType() == HiveParser.TOK_IFNOTEXISTS) {
    ...
    private void handleInsertStatementSpecPhase1(ASTNode ast, QBParseInfo qbp, Phase1Ctx ctx_1) throws SemanticException {
        ...
        if(ast.getType() == HiveParser.TOK_INSERT_INTO && tabColName != null && tabColName.getType() == HiveParser.TOK_TABCOLNAME) {
            ...
            for(Node col : tabColName.getChildren()) {
                assert ((ASTNode) col).getType() == HiveParser.Identifier : "expected token " + HiveParser.Identifier + " found " + ((ASTNode) col)
                        .getType();
            ...
            if(targetColNames.size() != targetColumns.size()) {
                throw new SemanticException(
                        generateErrorMessage(tabColName, "Duplicate column name detected in " + fullTableName + " table schema specification"));
            }
            ...
            /* We just checked the user specified schema columns among regular table column and found some which
             are not
            'regular'.  Now check is they are dynamic partition columns
              ...
              for "insert into multipart partition(c='1',d)(d,a) values(2,3);" we expect parse tree to look like
               this
               (TOK_INSERT_INTO
                (TOK_TAB
                    ...
                    if(tokPartSpec != null) {
                        for(Node n : tokPartSpec.getChildren()) {
                            ...
                            if(tokPartVal != null && tokPartVal.getType() == HiveParser.TOK_PARTVAL && tokPartVal.getChildCount() == 1) {
                                assert tokPartVal.getChild(0)
                                        .getType() == HiveParser.Identifier : "Expected " + "column name; " + "found tokType=" + tokPartVal.getType();
                ...
                if(!targetColumns.isEmpty()) {
                    //Found some columns in user specified schema which are neither regular not dynamic
                    // partition columns
                    throw new SemanticException(generateErrorMessage(tabColName, "'" + (targetColumns.size() == 1 ? targetColumns.iterator()
                            .next() : targetColumns) + "' " + "in" + " insert schema specification " + (targetColumns
                            .size() == 1 ? "is" : "are") + " " + "not found" + " among regular columns of " + fullTableName + " nor dynamic partition " + "columns."));
                }
            }
        }
    }
        ...
        for(String alias : qb.getTabAliases()) {
            ...
            if(cte != null) {
                if(ctesExpanded.contains(cteName)) {
                    throw new SemanticException(
                            "Recursive cte " + cteName + " detected (cycle: " + StringUtils.join(ctesExpanded, " -> ") + " -> " + cteName + ").");
                }
                cte.reference++;
        ...
        for(String alias : tabAliases) {
            ...
            if(tab.isView()) {
                ...
                if(viewsExpanded.contains(fullViewName)) {
                    throw new SemanticException("Recursive view " + fullViewName + " detected (cycle: " + StringUtils
                            .join(viewsExpanded, " -> ") + " -> " + fullViewName + ").");
                }
            ...
            if(!InputFormat.class.isAssignableFrom(tab.getInputFormatClass())) {
                throw new SemanticException(
                        generateErrorMessage(qb.getParseInfo().getSrcForAlias(alias), ErrorMsg.INVALID_INPUT_FORMAT_TYPE.getMsg()));
            }
            ...
            if(qb.getParseInfo().isAnalyzeCommand()) {
                ...
                if(ts.specType == SpecType.DYNAMIC_PARTITION) { // dynamic partitions
                    try {
                        ...
                        throw new SemanticException(
                                generateErrorMessage(qb.getParseInfo().getSrcForAlias(alias), "Cannot " + "get partitions for " + ts.partSpec), e);
                    }
                }
            ...
            switch(ast.getToken().getType()) {
                case HiveParser.TOK_TAB: {
                    ...
                    if(!ts.tableHandle.isNonNative() && !HiveOutputFormat.class.isAssignableFrom(outputFormatClass)) {
                        throw new SemanticException(
                                ErrorMsg.INVALID_OUTPUT_FORMAT_TYPE.getMsg(ast, "The class " + "is " + outputFormatClass.toString()));
                    }
                ...
                case HiveParser.TOK_DIR: {
                    ...
                    if((!qb.getParseInfo().getIsSubQ()) && (((ASTNode) ast.getChild(0)).getToken().getType() == HiveParser.TOK_TMP_FILE)) {
                        ...
                        if(qb.isCTAS() || qb.isMaterializedView()) {
                            ...
                            Path location;
                            // If the CTAS query does specify a location, use the table location, else use the
                            // db location
     ...
     * @return The strongest encrypted path. It may return NULL if there are not tables encrypted, or are not
     * HDFS tables.
            ...
            if(isPathReadOnly(tablePath)) {
                ...
                if(comparePathKeyStrength(tablePath, tmpPath) < 0) {
                    throw new HiveException(
                            "Read-only encrypted tables cannot be read " + "if the scratch " + "directory " + "is not encrypted (or encryption is weak)");
                } else {
                    stagingPath = tmpPath;
                }
            }
    ...
    private void replaceViewReferenceWithDefinition(QB qb, Table tab, String tab_name, String alias) throws SemanticException {
        ...
        ASTNode viewTree;
        final ASTNodeOrigin viewOrigin = new ASTNodeOrigin("VIEW", tab.getTableName(), tab.getViewExpandedText(), alias,
        ...
        try {
            ...
            if(!unparseTranslator.isEnabled() && (viewMask.isEnabled() && analyzeRewrite == null)) {
                viewTree = rewriteASTWithMaskAndFilter(viewMask, viewTree, ctx.getViewTokenRewriteStream(viewFullyQualifiedName), ctx, db,
                        tabNameToTabObject, ignoredTokens);
            }
        ...
        if((!this.skipAuthorization() && !qb.isInsideView() && HiveConf.getBoolVar(conf, HiveConf.ConfVars.HIVE_AUTHORIZATION_ENABLED)) || HiveConf
                .getBoolVar(conf, HiveConf.ConfVars.HIVE_STATS_COLLECT_SCANCOLS)) {
    ...
    @SuppressWarnings("nls")
    void parseJoinCondPopulateAlias(QBJoinTree joinTree, ASTNode condn, ArrayList<String> leftAliases, ArrayList<String> rightAliases,
            ArrayList<String> fields, Map<String, Operator> aliasToOpInfo) throws SemanticException {
    ...
    private void populateAliases(List<String> leftAliases, List<String> rightAliases, ASTNode condn, QBJoinTree joinTree,
     ...
     */
    void applyEqualityPredicateToQBJoinTree(QBJoinTree joinTree, JoinType type, List<String> leftSrc, ASTNode joinCond, ASTNode leftCondn,
            ASTNode rightCondn, List<String> leftCondAl1, List<String> leftCondAl2, List<String> rightCondAl1,
                ...
                if(type.equals(JoinType.LEFTOUTER) || type.equals(JoinType.FULLOUTER)) {
                    ...
                    if(rightCondAl1.size() != 0) {
                        ...
                        if(leftTree != null && leftTree.getNoOuterJoin()) {
                            String leftTreeRightSource = leftTree.getRightAliases() != null && leftTree.getRightAliases().length > 0 ? leftTree
                                    .getRightAliases()[0] : null;
                            ...
                            if(!leftHasRightReference && !rightHasRightReference) {
                                applyEqualityPredicateToQBJoinTree(leftTree, type, leftTreeLeftSrc, joinCond, leftCondn, rightCondn, leftCondAl1,
                                        leftCondAl2, rightCondAl1, rightCondAl2);
                                pushedDown = true;
                            } else if(!leftHasRightReference && rightHasRightReference && rightCondAl1.size() == 1) {
                                applyEqualityPredicateToQBJoinTree(leftTree, type, leftTreeLeftSrc, joinCond, leftCondn, rightCondn, leftCondAl1,
                                        leftCondAl2, rightCondAl2, rightCondAl1);
                                pushedDown = true;
                            } else if(leftHasRightReference && !rightHasRightReference && leftCondAl1.size() == 1) {
                                applyEqualityPredicateToQBJoinTree(leftTree, type, leftTreeLeftSrc, joinCond, leftCondn, rightCondn, leftCondAl2,
                                        leftCondAl1, rightCondAl1, rightCondAl2);
                                pushedDown = true;
                            }
     ...
     */
    @SuppressWarnings("rawtypes")
    private void parseJoinCondition(QBJoinTree joinTree, ASTNode joinCond, List<String> leftSrc, JoinType type,
            Map<String, Operator> aliasToOpInfo) throws SemanticException {
        if(joinCond == null) {
            return;
        }
        ...
        switch(joinCond.getToken().getType()) {
            ...
            default:
                ...
                for(int ci = childrenBegin; ci < joinCond.getChildCount(); ci++) {
                    parseJoinCondPopulateAlias(joinTree, (ASTNode) joinCond.getChild(ci), leftAlias.get(ci - childrenBegin),
                            rightAlias.get(ci - childrenBegin), null, aliasToOpInfo);
                }
    ...
    private void extractJoinCondsFromWhereClause(QBJoinTree joinTree, QB qb, String dest, ASTNode predicate,
        ...
        switch(predicate.getType()) {
            ...
            case HiveParser.EQUAL:
                ...
                JoinCond cond = joinTree.getJoinCond()[0];
                JoinType type = cond.getJoinType();
                applyEqualityPredicateToQBJoinTree(joinTree, type, leftSrc, predicate, leftCondn, rightCondn, leftCondAl1, leftCondAl2, rightCondAl1,
                        rightCondAl2);
                if(leftSrc.size() == 1) {
    ...
    private Operator genHavingPlan(String dest, QB qb, Operator input, Map<String, Operator> aliasToOpInfo) throws SemanticException {
        ...
        if(!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
            ...
            final boolean cubeRollupGrpSetPresent = (!qb.getParseInfo().getDestRollups().isEmpty() || !qb.getParseInfo().getDestGroupingSets()
                        ...
                        for(int i = 0; i < grpByAstExprs.size(); i++) {
                            ...
                            if(grpByExpr.toStringTree().equals(c.toStringTree())) {
                                ...
                                if(noneSet) {
                                    ...
                                    ParseDriver.adaptor
                                            .addChild(child1, ParseDriver.adaptor.create(HiveParser.Identifier, VirtualColumn.GROUPINGID.getName()));
                                }
                                ASTNode child2 = (ASTNode) ParseDriver.adaptor
                                        .create(HiveParser.IntegralLiteral, String.valueOf(IntMath.mod(-i - 1, grpByAstExprs.size())));
    ...
    @SuppressWarnings("nls")
    private Operator genFilterPlan(ASTNode searchCond, QB qb, Operator input, Map<String, Operator> aliasToOpInfo, boolean forHavingClause,
            boolean forGroupByClause) throws SemanticException {
        ...
        if(subQueriesInOriginalTree.size() > 0) {
            ...
            if(qb.getSubQueryPredicateDef() != null) {
                throw new SemanticException(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION
                        .getMsg(subQueriesInOriginalTree.get(0), "Nested SubQuery expressions are not supported."));
            }
            ...
            if(subQueriesInOriginalTree.size() > 1) {
                ...
                throw new SemanticException(
                        ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(subQueriesInOriginalTree.get(1), "Only 1 SubQuery expression is supported."));
            }
            ...
            for(int i = 0; i < subQueries.size(); i++) {
                 ...
                 */
                if(subQuery.getOperator().getType() != SubQueryType.EXISTS && subQuery.getOperator().getType() != SubQueryType.NOT_EXISTS && sqRR
                        .getColumnInfos().size() - subQuery.getNumOfCorrelationExprsAddedToSQSelect() > 1) {
                    throw new SemanticException(
                            ErrorMsg.INVALID_SUBQUERY_EXPRESSION.getMsg(subQueryAST, "SubQuery can " + "contain only 1 item in Select List."));
                }
    ...
    private Operator genFilterPlan(QB qb, ASTNode condn, Operator input, boolean useCaching) throws SemanticException {
        ...
        Operator output = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new FilterDesc(filterCond, false), new RowSchema(inputRR.getColumnInfos()), input), inputRR);
    ...
    private Operator genNotNullFilterForJoinSourcePlan(QB qb, Operator input, QBJoinTree joinTree, ExprNodeDesc[] joinKeys) throws SemanticException {
        ...
        for(int i = 0; i < joinKeys.length; i++) {
            if(nullSafes.get(i) || (joinKeys[i] instanceof ExprNodeColumnDesc && ((ExprNodeColumnDesc) joinKeys[i])
    ...
    Integer genColListRegex(String colRegex, String tabAlias, ASTNode sel, ArrayList<ExprNodeDesc> col_list, HashSet<ColumnInfo> excludeCols,
            RowResolver input, RowResolver colSrcRR, Integer pos, RowResolver output, List<String> aliases,
                ...
                for(Map.Entry<String, ColumnInfo> entry : chosenMap.entrySet()) {
                    ...
                    if(oColInfo == null) {
                        ExprNodeColumnDesc expr = new ExprNodeColumnDesc(colInfo.getType(), name, colInfo.getTabAlias(), colInfo.getIsVirtualCol(),
                                colInfo.isSkewedCol());
                        col_list.add(expr);
                        oColInfo = new ColumnInfo(getColumnInternalName(pos), colInfo.getType(), colInfo.getTabAlias(), colInfo.getIsVirtualCol(),
                    ...
                    if(ensureUniqueCols) {
                        if(!output.putWithCheck(tmp[0], tmp[1], null, oColInfo)) {
                            throw new CalciteSemanticException(
                                    "Cannot add column to RR: " + tmp[0] + "." + tmp[1] + " => " + oColInfo + " due to duplication, see previous warnings",
        ...
        for(String alias : aliases) {
            ...
            for(Map.Entry<String, ColumnInfo> entry : fMap.entrySet()) {
                ...
                if(input != colSrcRR) {
                    ...
                    if(colInfo == null) {
                        LOG.error(
                                "Cannot find colInfo for " + tabAlias + "." + tmp[1] + ", derived from [" + colSrcRR + "]," + " in [" + input + "]");
    ...
    private Operator genScriptPlan(ASTNode trfm, QB qb, Operator input) throws SemanticException {
            ...
            if(outputColNames) {
                ...
                for(int i = 0; i < ccount; ++i) {
                    ...
                    ColumnInfo colInfo = new ColumnInfo(intName,
                            TypeInfoUtils.getTypeInfoFromTypeString(getTypeStringFromAST((ASTNode) child.getChild(1))), null, false);
                    colInfo.setAlias(colAlias);
                    outputCols.add(colInfo);
                }
            }
        ...
        if(trfm.getChild(inputSerDeNum).getChildCount() > 0) {
            inInfo = getTableDescFromSerDe((ASTNode) (((ASTNode) trfm.getChild(inputSerDeNum))).getChild(0), inpColumns.toString(),
        ...
        if(trfm.getChild(outputSerDeNum).getChildCount() > 0) {
            outInfo = getTableDescFromSerDe((ASTNode) (((ASTNode) trfm.getChild(outputSerDeNum))).getChild(0), columns.toString(),
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new ScriptDesc(fetchFilesNotInLocalFilesystem(stripQuotes(trfm.getChild(execPos).getText())), inInfo, inRecordWriter, outInfo,
                        outRecordReader, errRecordReader, errInfo), new RowSchema(out_rwsch.getColumnInfos()), input), out_rwsch);
    ...
    @SuppressWarnings("nls")
    private Operator<?> genSelectPlan(String dest, ASTNode selExprList, QB qb, Operator<?> input, Operator<?> inputForSelectStar,
        ...
        for(int i = startPosn; i < exprList.getChildCount(); ++i) {
            ...
            if(expr.getType() == HiveParser.TOK_ALLCOLREF) {
                int initPos = pos;
                pos = genColListRegex(".*", expr.getChildCount() == 0 ? null : getUnescapedName((ASTNode) expr.getChild(0)).toLowerCase(), expr,
                        col_list, null, inputRR, starRR, pos, out_rwsch, qb.getAliases(), false);
                if(unparseTranslator.isEnabled()) {
                    offset += pos - initPos - 1;
                }
                selectStar = true;
            } else if(expr.getType() == HiveParser.TOK_TABLE_OR_COL && !hasAsClause && !inputRR.getIsExprResolver() && isRegex(
                    unescapeIdentifier(expr.getChild(0).getText()), conf)) {
                // In case the expression is a regex COL.
                // This can only happen without AS clause
                // We don't allow this for ExprResolver - the Group By case
                pos = genColListRegex(unescapeIdentifier(expr.getChild(0).getText()), null, expr, col_list, null, inputRR, starRR, pos, out_rwsch,
                        qb.getAliases(), false);
            } else if(expr.getType() == HiveParser.DOT && expr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL && inputRR
                    .hasTableAlias(unescapeIdentifier(expr.getChild(0).getChild(0).getText().toLowerCase())) && !hasAsClause && !inputRR
                    .getIsExprResolver() && isRegex(unescapeIdentifier(expr.getChild(1).getText()), conf)) {
                // In case the expression is TABLE.COL (col can be regex).
                // This can only happen without AS clause
                // We don't allow this for ExprResolver - the Group By case
                pos = genColListRegex(unescapeIdentifier(expr.getChild(1).getText()),
                        unescapeIdentifier(expr.getChild(0).getChild(0).getText().toLowerCase()), expr, col_list, null, inputRR, starRR, pos,
        ...
        Operator output = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new SelectDesc(col_list, columnNames, selectStar), new RowSchema(out_rwsch.getColumnInfos()), input),
                out_rwsch);
     ...
     */
    public RowResolver handleInsertStatementSpec(List<ExprNodeDesc> col_list, String dest, RowResolver outputRR, RowResolver inputRR, QB qb,
        ...
        if(targetTableSchema.size() != col_list.size()) {
            ...
            throw new SemanticException(generateErrorMessage(selExprList,
                    "Expected " + targetTableSchema.size() + " " + "columns for " + dest + (target != null ? "/" + target
                            .getCompleteName() : (partition != null ? "/" + partition.getCompleteName() : "")) + "; select produces " + col_list
                            .size() + " columns"));
        }
     ...
     */
    public static GenericUDAFEvaluator getGenericUDAFEvaluator(String aggName, ArrayList<ExprNodeDesc> aggParameters, ASTNode aggTree,
     ...
     */
    public static GenericUDAFInfo getGenericUDAFInfo(GenericUDAFEvaluator evaluator, GenericUDAFEvaluator.Mode emode,
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanGroupByOperator(QBParseInfo parseInfo, String dest, Operator input, ReduceSinkOperator rs, GroupByDesc.Mode mode,
            Map<String, GenericUDAFEvaluator> genericUDAFEvaluators) throws SemanticException {
        ...
        for(Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            for(int i = 1; i < value.getChildCount(); i++) {
                ...
                if(isDistinct && lastKeyColName != null) {
                    ...
                    paraExpression = Utilities.ReduceField.KEY.name() + "." + lastKeyColName + ":" + numDistinctUDFs + "." + getColumnInternalName(
                            i - 1);

                }

                ExprNodeDesc expr = new ExprNodeColumnDesc(paraExprInfo.getType(), paraExpression, paraExprInfo.getTabAlias(),
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations, false, groupByMemoryUsage, memoryThreshold, null, false, -1,
                        numDistinctUDFs > 0), new RowSchema(groupByOutputRowResolver.getColumnInfos()), input), groupByOutputRowResolver);
    ...
    private void addGroupingSetKey(List<ExprNodeDesc> groupByKeys, RowResolver groupByInputRowResolver, RowResolver groupByOutputRowResolver,
            List<String> outputColumnNames, Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
    ...
    private void processGroupingSetReduceSinkOperator(RowResolver reduceSinkInputRowResolver, RowResolver reduceSinkOutputRowResolver,
            List<ExprNodeDesc> reduceKeys, List<String> outputKeyColumnNames, Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanGroupByOperator1(QBParseInfo parseInfo, String dest, Operator reduceSinkOperatorInfo, GroupByDesc.Mode mode,
            Map<String, GenericUDAFEvaluator> genericUDAFEvaluators, List<Integer> groupingSets, boolean groupingSetsPresent,
            ...
            if(isDistinct) {
                ...
                for(int i = 1; i < value.getChildCount(); i++) {
                    ...
                    if(isDistinct && lastKeyColName != null) {
                        // if aggr is distinct, the parameter is name is constructed as
                        // KEY.lastKeyColName:<tag>._colx
                        paraExpression = Utilities.ReduceField.KEY
                                .name() + "." + lastKeyColName + ":" + numDistinctUDFs + "." + getColumnInternalName(i - 1);
                    }
                ...
                String paraExpression = paraExprInfo.getInternalName();
                assert (paraExpression != null);
                aggParameters.add(new ExprNodeColumnDesc(paraExprInfo.getType(), paraExpression, paraExprInfo.getTabAlias(),
                        paraExprInfo.getIsVirtualCol()));
            }
            if(isDistinct) {
                numDistinctUDFs++;
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations, groupByMemoryUsage, memoryThreshold, groupingSets,
                        groupingSetsPresent && groupingSetsNeedAdditionalMRJob, groupingSetsPosition, containsDistinctAggr),
                new RowSchema(groupByOutputRowResolver.getColumnInfos()), reduceSinkOperatorInfo), groupByOutputRowResolver);
        op.setColumnExprMap(colExprMap);
        return op;
    }
     ...
     */
    private void createNewGroupingKey(List<ExprNodeDesc> groupByKeys, List<String> outputColumnNames, RowResolver groupByOutputRowResolver,
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanMapGroupByOperator(QB qb, String dest, List<ASTNode> grpByExprs, Operator inputOperatorInfo, GroupByDesc.Mode mode,
            Map<String, GenericUDAFEvaluator> genericUDAFEvaluators, List<Integer> groupingSetKeys,
        ...
        float memoryThreshold = HiveConf.getFloatVar(conf, HiveConf.ConfVars.HIVEMAPAGGRMEMORYTHRESHOLD);
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations, false, groupByMemoryUsage, memoryThreshold, groupingSetKeys,
                        groupingSetsPresent, groupingSetsPosition, containsDistinctAggr), new RowSchema(groupByOutputRowResolver.getColumnInfos()),
     ...
     */
    @SuppressWarnings("nls")
    private ReduceSinkOperator genGroupByPlanReduceSinkOperator(QB qb, String dest, Operator inputOperatorInfo, List<ASTNode> grpByExprs,
            int numPartitionFields, boolean changeNumPartitionFields, int numReducers, boolean mapAggrDone,
            boolean groupingSetsPresent) throws SemanticException {
        ...
        ArrayList<ExprNodeDesc> reduceKeys = getReduceKeysForReduceSink(grpByExprs, dest, reduceSinkInputRowResolver, reduceSinkOutputRowResolver,
                outputKeyColumnNames, colExprMap);
        ...
        if(groupingSetsPresent) {
            // Process grouping set for the reduce sink operator
            processGroupingSetReduceSinkOperator(reduceSinkInputRowResolver, reduceSinkOutputRowResolver, reduceKeys, outputKeyColumnNames,
                    colExprMap);
        ...
        List<List<Integer>> distinctColIndices = getDistinctColIndicesForReduceSink(parseInfo, dest, reduceKeys, reduceSinkInputRowResolver,
                reduceSinkOutputRowResolver, outputKeyColumnNames, colExprMap);

        ArrayList<ExprNodeDesc> reduceValues = new ArrayList<ExprNodeDesc>();
        HashMap<String, ASTNode> aggregationTrees = parseInfo.getAggregationExprsForClause(dest);

        if(!mapAggrDone) {
            getReduceValuesForReduceSinkNoMapAgg(parseInfo, dest, reduceSinkInputRowResolver, reduceSinkOutputRowResolver, outputValueColumnNames,
                    reduceValues, colExprMap);
        } else {
        ...
        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(OperatorFactory.getAndMakeChild(PlanUtils
                        .getReduceSinkDesc(reduceKeys, groupingSetsPresent ? keyLength + 1 : keyLength, reduceValues, distinctColIndices,
                                outputKeyColumnNames, outputValueColumnNames, true, -1, numPartitionFields, numReducers, AcidUtils.Operation.NOT_ACID),
                new RowSchema(reduceSinkOutputRowResolver.getColumnInfos()), inputOperatorInfo), reduceSinkOutputRowResolver);
        rsOp.setColumnExprMap(colExprMap);
        return rsOp;
    }

    private ArrayList<ExprNodeDesc> getReduceKeysForReduceSink(List<ASTNode> grpByExprs, String dest, RowResolver reduceSinkInputRowResolver,
    ...
    private List<List<Integer>> getDistinctColIndicesForReduceSink(QBParseInfo parseInfo, String dest, List<ExprNodeDesc> reduceKeys,
            RowResolver reduceSinkInputRowResolver, RowResolver reduceSinkOutputRowResolver, List<String> outputKeyColumnNames,
            Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
    ...
    private void getReduceValuesForReduceSinkNoMapAgg(QBParseInfo parseInfo, String dest, RowResolver reduceSinkInputRowResolver,
            RowResolver reduceSinkOutputRowResolver, List<String> outputValueColumnNames, ArrayList<ExprNodeDesc> reduceValues,
            Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
        HashMap<String, ASTNode> aggregationTrees = parseInfo.getAggregationExprsForClause(dest);
            ...
            for(int i = 1; i < value.getChildCount(); i++) {
                ...
                if(reduceSinkOutputRowResolver.getExpression(parameter) == null) {
                    ...
                    reduceSinkOutputRowResolver
                            .putExpression(parameter, new ColumnInfo(field, reduceValues.get(reduceValues.size() - 1).getTypeInfo(), null, false));
    ...
    private ReduceSinkOperator genCommonGroupByPlanReduceSinkOperator(QB qb, List<String> dests,
        ...
        for(String destination : dests) {
            ...
            getReduceValuesForReduceSinkNoMapAgg(parseInfo, destination, reduceSinkInputRowResolver, reduceSinkOutputRowResolver,
                    outputValueColumnNames, reduceValues, colExprMap);
        ...
        ReduceSinkDesc rsDesc = PlanUtils
                .getReduceSinkDesc(reduceKeys, keyLength, reduceValues, distinctColIndices, outputKeyColumnNames, outputValueColumnNames, true, -1,
                        keyLength, numReducers, AcidUtils.Operation.NOT_ACID);

        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(
                OperatorFactory.getAndMakeChild(rsDesc, new RowSchema(reduceSinkOutputRowResolver.getColumnInfos()), inputOperatorInfo),
                reduceSinkOutputRowResolver);
        rsOp.setColumnExprMap(colExprMap);
        return rsOp;
    }
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanReduceSinkOperator2MR(QBParseInfo parseInfo, String dest, Operator groupByOperatorInfo, int numPartitionFields,
            int numReducers, boolean groupingSetsPresent) throws SemanticException {
        ...
        if(groupingSetsPresent) {
            ...
            processGroupingSetReduceSinkOperator(reduceSinkInputRowResolver2, reduceSinkOutputRowResolver2, reduceKeys, outputColumnNames,
                    colExprMap);
        }
        ...
        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(OperatorFactory.getAndMakeChild(PlanUtils
                        .getReduceSinkDesc(reduceKeys, reduceValues, outputColumnNames, true, -1, numPartitionFields, numReducers,
                                AcidUtils.Operation.NOT_ACID), new RowSchema(reduceSinkOutputRowResolver2.getColumnInfos()), groupByOperatorInfo),
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanGroupByOperator2MR(QBParseInfo parseInfo, String dest, Operator reduceSinkOperatorInfo2, GroupByDesc.Mode mode,
            Map<String, GenericUDAFEvaluator> genericUDAFEvaluators, boolean groupingSetsPresent) throws SemanticException {
        ...
        for(Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            assert (paraExpression != null);
            aggParameters
                    .add(new ExprNodeColumnDesc(paraExprInfo.getType(), paraExpression, paraExprInfo.getTabAlias(), paraExprInfo.getIsVirtualCol()));
            ...
            GenericUDAFInfo udaf = getGenericUDAFInfo(genericUDAFEvaluator, amode, aggParameters);
            aggregations.add(new AggregationDesc(aggName.toLowerCase(), udaf.genericUDAFEvaluator, udaf.convertedParameters,
                    (mode != GroupByDesc.Mode.FINAL && value.getToken().getType() == HiveParser.TOK_FUNCTIONDI), amode));
            String field = getColumnInternalName(groupByKeys.size() + aggregations.size() - 1);
            outputColumnNames.add(field);
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations, false, groupByMemoryUsage, memoryThreshold, null, false,
                        groupingSetsPosition, containsDistinctAggr), new RowSchema(groupByOutputRowResolver2.getColumnInfos()),
    ...
    private Operator genGroupByPlan1MR(String dest, QB qb, Operator input) throws SemanticException {
        ...
        ReduceSinkOperator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb, dest, input, grpByExprs, grpByExprs.size(), false,
                numReducers, false, false);

        // ////// 2. Generate GroupbyOperator
        Operator groupByOperatorInfo = genGroupByPlanGroupByOperator(parseInfo, dest, reduceSinkOperatorInfo, reduceSinkOperatorInfo,
                GroupByDesc.Mode.COMPLETE, null);
    ...
    private Operator genGroupByPlan1ReduceMultiGBY(List<String> dests, QB qb, Operator input,
        ...
        Operator forwardOp = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new ForwardDesc(), new RowSchema(reduceSinkOperatorInfoRR.getColumnInfos()), reduceSinkOperatorInfo),
        ...
        for(String dest : dests) {
            ...
            Operator groupByOperatorInfo = genGroupByPlanGroupByOperator(parseInfo, dest, curr, reduceSinkOperatorInfo, GroupByDesc.Mode.COMPLETE,
                    null);
    ...
    private Operator genGroupByPlan2MR(String dest, QB qb, Operator input) throws SemanticException {
        ...
        ReduceSinkOperator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb, dest, input, grpByExprs,
                (parseInfo.getDistinctFuncExprsForClause(dest).isEmpty() ? -1 : Integer.MAX_VALUE), false, -1, false, false);

        // ////// 2. Generate GroupbyOperator
        Map<String, GenericUDAFEvaluator> genericUDAFEvaluators = new LinkedHashMap<String, GenericUDAFEvaluator>();
        GroupByOperator groupByOperatorInfo = (GroupByOperator) genGroupByPlanGroupByOperator(parseInfo, dest, reduceSinkOperatorInfo,
                reduceSinkOperatorInfo, GroupByDesc.Mode.PARTIAL1, genericUDAFEvaluators);
    ...
    void checkExpressionsForGroupingSet(List<ASTNode> grpByExprs, List<ASTNode> distinctGrpByExprs, Map<String, ASTNode> aggregationTrees,
    ...
    private Operator genGroupByPlanMapAggrNoSkew(String dest, QB qb, Operator inputOperatorInfo) throws SemanticException {
        ...
        if(groupingSetsPresent) {
            checkExpressionsForGroupingSet(grpByExprs, parseInfo.getDistinctFuncExprsForClause(dest), parseInfo.getAggregationExprsForClause(dest),
        ...
        GroupByOperator groupByOperatorInfo = (GroupByOperator) genGroupByPlanMapGroupByOperator(qb, dest, grpByExprs, inputOperatorInfo,
                GroupByDesc.Mode.HASH, genericUDAFEvaluators, groupingSets, groupingSetsPresent && !groupingSetsNeedAdditionalMRJob);
        ...
        Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb, dest, groupByOperatorInfo, grpByExprs, grpByExprs.size(), true,
                numReducers, true, groupingSetsPresent && !groupingSetsNeedAdditionalMRJob);
        ...
        if(!groupingSetsPresent || !groupingSetsNeedAdditionalMRJob) {
            ...
            // table.
            Operator groupByOperatorInfo2 = genGroupByPlanGroupByOperator1(parseInfo, dest, reduceSinkOperatorInfo, GroupByDesc.Mode.PARTIALS,
                    genericUDAFEvaluators, groupingSets, groupingSetsPresent, groupingSetsNeedAdditionalMRJob);

            // ////// Generate ReduceSinkOperator2
            Operator reduceSinkOperatorInfo2 = genGroupByPlanReduceSinkOperator2MR(parseInfo, dest, groupByOperatorInfo2, grpByExprs.size() + 1,
                    numReducers, groupingSetsPresent);
    ...
    private Operator genGroupByPlanMapAggr2MR(String dest, QB qb, Operator inputOperatorInfo) throws SemanticException {
        ...
        Map<String, GenericUDAFEvaluator> genericUDAFEvaluators = new LinkedHashMap<String, GenericUDAFEvaluator>();
        GroupByOperator groupByOperatorInfo = (GroupByOperator) genGroupByPlanMapGroupByOperator(qb, dest, grpByExprs, inputOperatorInfo,
                GroupByDesc.Mode.HASH, genericUDAFEvaluators, groupingSets, groupingSetsPresent);
        ...
        if(!optimizeMapAggrGroupBy(dest, qb)) {
            ...
            // ////// Generate ReduceSink Operator
            Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb, dest, groupByOperatorInfo, grpByExprs,
                    distinctFuncExprs.isEmpty() ? -1 : Integer.MAX_VALUE, false, -1, true, groupingSetsPresent);

            // ////// Generate GroupbyOperator for a partial aggregation
            Operator groupByOperatorInfo2 = genGroupByPlanGroupByOperator1(parseInfo, dest, reduceSinkOperatorInfo, GroupByDesc.Mode.PARTIALS,
                    genericUDAFEvaluators, groupingSets, groupingSetsPresent, false);
            ...
            // ////// Generate ReduceSinkOperator2
            Operator reduceSinkOperatorInfo2 = genGroupByPlanReduceSinkOperator2MR(parseInfo, dest, groupByOperatorInfo2, grpByExprs.size(),
                    numReducers, groupingSetsPresent);
            ...
            // ////// Generate ReduceSink Operator
            Operator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb, dest, groupByOperatorInfo, grpByExprs, grpByExprs.size(), false, 1,
                    true, groupingSetsPresent);
    ...
    @SuppressWarnings("nls")
    private Operator genBucketingSortingDest(String dest, Operator input, QB qb, TableDesc table_desc, Table dest_tab,
        ...
        if(enforceBucketing) {
            ...
            input = genReduceSinkPlan(input, partnCols, sortCols, order.toString(), nullOrder.toString(), maxReducers,
    ...
    private void genPartnCols(String dest, Operator input, QB qb, TableDesc table_desc, Table dest_tab,
            SortBucketRSCtx ctx) throws SemanticException {
        boolean enforceBucketing = false;
    ...
    protected Operator genFileSinkPlan(String dest, QB qb, Operator input) throws SemanticException {
            ...
            case QBMetaData.DEST_TABLE: {
                ...
                if(parts != null && parts.size() > 0) { // table is partitioned
                    if(partSpec == null || partSpec.size() == 0) { // user did NOT specify partition
                        throw new SemanticException(
                                generateErrorMessage(qb.getParseInfo().getDestForClause(dest), ErrorMsg.NEED_PARTITION_ERROR.getMsg()));
                    }
                    dpCtx = qbm.getDPCtx(dest);
                    if(dpCtx == null) {
                        dest_tab.validatePartColumnNames(partSpec, false);
                        dpCtx = new DynamicPartitionCtx(dest_tab, partSpec, conf.getVar(HiveConf.ConfVars.DEFAULTPARTITIONNAME),
                    ...
                    if(!HiveConf.getBoolVar(conf, HiveConf.ConfVars.DYNAMICPARTITIONING)) { // allow DP
                        throw new SemanticException(
                                generateErrorMessage(qb.getParseInfo().getDestForClause(dest), ErrorMsg.DYNAMIC_PARTITION_DISABLED.getMsg()));
                    }
                ...
                if((dpCtx != null) && (dpCtx.getNumDPCols() >= 0)) {
                    ...
                    // partitions currently, we need to store dummy partitions
                        ...
                        try {
                            ...
                            DummyPartition p = new DummyPartition(dest_tab, dest_tab.getDbName() + "@" + dest_tab.getTableName() + "@" + ppath,
                                    partSpec);
            ...
            case QBMetaData.DEST_PARTITION: {
                ...
                if(dest_tab.isImmutable() && qb.getParseInfo().isInsertIntoTable(dest_tab.getDbName(), dest_tab.getTableName())) {
                    try {
                        ...
                        LOG.warn("Error while trying to determine if immutable table partition has any data : " + dest_tab
                                .getTableName() + " : " + partPath);
                        throw new SemanticException(ErrorMsg.INSERT_INTO_IMMUTABLE_TABLE.getMsg(ioe.getMessage()));
                    }
                }
                ...
                lbCtx = constructListBucketingCtx(dest_part.getSkewedColNames(), dest_part.getSkewedColValues(),
                        dest_part.getSkewedColValueLocationMaps(), dest_part.isStoredAsSubDirectories(), conf);
                AcidUtils.Operation acidOp = AcidUtils.Operation.NOT_ACID;
                if(destTableIsAcid) {
                    acidOp = getAcidType(table_desc.getOutputFileFormatClass(), dest);
                    checkAcidConstraints(qb, table_desc, dest_tab);
        ...
        boolean canBeMerged = (dest_tab == null || !((dest_tab.getNumBuckets() > 0) || (dest_tab.getSortCols() != null && dest_tab.getSortCols()
                .size() > 0)));
        ...
        FileSinkDesc fileSinkDesc = new FileSinkDesc(queryTmpdir, table_desc, conf.getBoolVar(HiveConf.ConfVars.COMPRESSRESULT), currentTableId,
                rsCtx.isMultiFileSpray(), canBeMerged, rsCtx.getNumFiles(), rsCtx.getTotalFiles(), rsCtx.getPartnCols(), dpCtx, dest_path);
        ...
        if(destTableIsAcid) {
            AcidUtils.Operation wt = updating(dest) ? AcidUtils.Operation.UPDATE : (deleting(
                    dest) ? AcidUtils.Operation.DELETE : AcidUtils.Operation.INSERT);
        ...
        if(isHiveServerQuery && null != table_desc && table_desc.getSerdeClassName()
                .equalsIgnoreCase(ThriftJDBCBinarySerDe.class.getName()) && HiveConf
                .getBoolVar(conf, HiveConf.ConfVars.HIVE_SERVER2_THRIFT_RESULTSET_SERIALIZE_IN_TASKS)) {
        ...
        if(dest_tab != null && conf.getBoolVar(ConfVars.HIVESTATSAUTOGATHER) && conf
                .getBoolVar(ConfVars.HIVESTATSCOLAUTOGATHER) && ColumnStatsAutoGatherContext.canRunAutogatherStats(fso)) {
            if(dest_type.intValue() == QBMetaData.DEST_TABLE) {
                ...
                genAutoColumnStatsGatheringPipeline(qb, table_desc, dest_part.getSpec(), input,
                        qb.getParseInfo().isInsertIntoTable(dest_tab.getDbName(), dest_tab.getTableName()));
    ...
    private void genAutoColumnStatsGatheringPipeline(QB qb, TableDesc table_desc, Map<String, String> partSpec, Operator curr,
    ...
    */
     ...
     */
    Operator genConversionSelectOperator(String dest, QB qb, Operator input, TableDesc table_desc,
        ...
        if(!isMetaDataSerDe && !deleting(dest)) {
            ...
            for(int i = 0; i < columnNumber; i++) {
                ...
                ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo, rowFields.get(rowFieldsOffset).getInternalName(), "", false,
        ...
        if(converted) {
            ...
            input = putOpInsertMap(
                    OperatorFactory.getAndMakeChild(new SelectDesc(expressions, colNames), new RowSchema(rowResolver.getColumnInfos()), input),
                    rowResolver);
    ...
    private Operator genUDTFPlan(GenericUDTF genericUDTF, String outputTableAlias, ArrayList<String> colAliases, QB qb, Operator input,
            boolean outerLV) throws SemanticException {
        ...
        if(numUdtfCols != numSuppliedAliases) {
            throw new SemanticException(
                    ErrorMsg.UDTF_ALIAS_MISMATCH.getMsg("expected " + numUdtfCols + " " + "aliases" + " " + "but got " + numSuppliedAliases));
        }
        ...
        for(StructField sf : outputOI.getAllStructFieldRefs()) {
            ...
            ColumnInfo col = new ColumnInfo(sf.getFieldName(), TypeInfoUtils.getTypeInfoFromObjectInspector(sf.getFieldObjectInspector()),
        ...
        Operator<?> udtf = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new UDTFDesc(genericUDTF, outerLV), new RowSchema(out_rwsch.getColumnInfos()), input), out_rwsch);
        return udtf;
    }
    ...
    private ArrayList<ExprNodeDesc> getPartitionColsFromBucketCols(String dest, QB qb, Table tab, TableDesc table_desc, Operator input,
    ...
    private ArrayList<ExprNodeDesc> genConvertCol(String dest, QB qb, Table tab, TableDesc table_desc, Operator input, List<Integer> posns,
            boolean convert) throws SemanticException {
        StructObjectInspector oi = null;
        try {
            Deserializer deserializer = table_desc.getDeserializerClass().newInstance();
        ...
        for(Integer posn : posns) {
            ...
            ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo, rowFields.get(posn).getInternalName(), rowFields.get(posn).getTabAlias(),
    ...
    private ArrayList<ExprNodeDesc> getSortCols(String dest, QB qb, Table tab, TableDesc table_desc, Operator input,
            boolean convert) throws SemanticException {
    ...
    private Operator genReduceSinkPlan(Operator<?> input, ArrayList<ExprNodeDesc> partitionCols, ArrayList<ExprNodeDesc> sortCols, String sortOrder,
    ...
    private Operator genReduceSinkPlan(Operator<?> input, ArrayList<ExprNodeDesc> partitionCols, ArrayList<ExprNodeDesc> sortCols, String sortOrder,
        ...
        ReduceSinkDesc rsdesc = PlanUtils
                .getReduceSinkDesc(newSortCols, valueCols, outputColumns, false, -1, partitionCols, newSortOrder.toString(), newNullOrder.toString(),
                        numReducers, acidOp);
    ...
    private Operator genJoinOperatorChildren(QBJoinTree join, Operator left, Operator[] right, HashSet<Integer> omitOpts,
        ...
        JoinOperator joinOp = (JoinOperator) OperatorFactory
                .getAndMakeChild(getOpContext(), desc, new RowSchema(outputRR.getColumnInfos()), rightOps);
    ...
    private Operator genJoinReduceSinkChild(QB qb, ExprNodeDesc[] joinKeys, Operator<?> child, String[] srcs, int tag) throws SemanticException {
        ...
        ReduceSinkDesc rsDesc = PlanUtils
                .getReduceSinkDesc(reduceKeys, reduceValues, outputColumns, false, tag, reduceKeys.size(), numReds, AcidUtils.Operation.NOT_ACID);

        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(
                OperatorFactory.getAndMakeChild(rsDesc, new RowSchema(outputRR.getColumnInfos()), child), outputRR);
        List<String> keyColNames = rsDesc.getOutputKeyColumnNames();
        for(int i = 0; i < keyColNames.size(); i++) {
    ...
    private Operator insertSelectForSemijoin(ArrayList<ASTNode> fields, Operator input) throws SemanticException {
        ...
        Operator output = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new SelectDesc(colList, outputColumnNames, false), new RowSchema(outputRR.getColumnInfos()), input),
                outputRR);
    ...
    private Operator genMapGroupByForSemijoin(QB qb, ArrayList<ASTNode> fields, // the
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(
                new GroupByDesc(mode, outputColumnNames, groupByKeys, aggregations, false, groupByMemoryUsage, memoryThreshold, null, false, -1,
                        false), new RowSchema(groupByOutputRowResolver.getColumnInfos()), inputOperatorInfo), groupByOutputRowResolver);
        ...
        for(int i = 0; i < joinParseTree.getChildCount(); i++) {
            ...
            switch(child.getToken().getType()) {
                case HiveParser.TOK_TABREF:
                    ...
                    String alias = child.getChildCount() == 1 ? tableName : unescapeIdentifier(
                            child.getChild(child.getChildCount() - 1).getText().toLowerCase());
     ...
     */
    private QBJoinTree genSQJoinTree(QB qb, ISubQueryJoinInfo subQuery, Operator joiningOp,
            Map<String, Operator> aliasToOpInfo) throws SemanticException {
        QBJoinTree joinTree = new QBJoinTree();
    ...
    private QBJoinTree genJoinTree(QB qb, ASTNode joinParseTree, Map<String, Operator> aliasToOpInfo) throws SemanticException {
        ...
        if(!isValidLeftToken && !isJoinLeftToken) {
            ...
            throw new SemanticException(
                    "Invalid token on the right side of the join: " + right.getToken().getText() + "; please rewrite your query" + advice);
        }
    ...
    private static boolean isValidJoinSide(ASTNode right) {
        return (right.getToken().getType() == HiveParser.TOK_TABREF) || (right.getToken().getType() == HiveParser.TOK_SUBQUERY) || (right.getToken()
                .getType() == HiveParser.TOK_PTBLFUNCTION);
    }
    ...
    private void mergeJoinTree(QB qb) {
        ...
        for(int i = trees.size() - 1; i >= 0; i--) {
            ...
            for(int j = i - 1; j >= 0 && continueScanning; j--) {
                ...
                if((!node.getNoOuterJoin() && node.getPostJoinFilters().size() != 0) || (!target.getNoOuterJoin() && target.getPostJoinFilters()
                        .size() != 0)) {
    ...
    private Operator genSelectAllDesc(Operator input) throws SemanticException {
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new SelectDesc(colList, columnNames, true), outputRR.getRowSchema(), input),
                outputRR);
    ...
    private Operator genBodyPlan(QB qb, Operator input, Map<String, Operator> aliasToOpInfo) throws SemanticException {
        ...
        if(!commonGroupByDestGroups.isEmpty()) {
            ...
            for(List<String> commonGroupByDestGroup : commonGroupByDestGroups) {
                ...
                if(commonGroupByDestGroup.size() == 1 || (qbp.getAggregationExprsForClause(firstDest).size() == 0 && getGroupByForClause(qbp,
                        firstDest).size() == 0) || conf.getBoolVar(HiveConf.ConfVars.HIVEGROUPBYSKEW) || !conf
                        .getBoolVar(HiveConf.ConfVars.HIVEMULTIGROUPBYSINGLEREDUCER)) {
                    ...
                    for(String dest : commonGroupByDestGroup) {
                        ...
                        if((qbp.getAggregationExprsForClause(dest).size() != 0 || getGroupByForClause(qbp, dest).size() > 0) && (qbp
                                .getSelForClause(dest).getToken().getType() != HiveParser.TOK_SELECTDI || qbp
                                .getWindowingExprsForClause(dest) == null)) {
                            ...
                            // not be included in the group by; (2) add the fully qualified column names to
                            // unParseTranslator
                            // so that view is supported. The drawback is that an additional SEL op is added. If
                            // it is
                            // not necessary, it will be removed by NonBlockingOpDeDupProc Optimizer because it
                            // will match
                            // SEL%SEL% rule.
                            ASTNode selExprList = qbp.getSelForClause(dest);
                            if(selExprList.getToken().getType() == HiveParser.TOK_SELECTDI && selExprList.getChildCount() == 1 && selExprList
                                    .getChild(0).getChildCount() == 1) {
                        ...
                        if(LOG.isDebugEnabled()) {
                            LOG.debug("RR before GB " + opParseCtx.get(gbySource).getRowResolver() + " after GB " + opParseCtx.get(curr)
                                    .getRowResolver());
                        }
    ...
    private Map<String, Operator<? extends OperatorDesc>> createInputForDests(QB qb, Operator<? extends OperatorDesc> input,
            Set<String> dests) throws SemanticException {
    ...
    private Operator genPostGroupByBodyPlan(Operator curr, String dest, QB qb, Map<String, Operator> aliasToOpInfo,
        ...
        if(queryProperties.hasWindowing() && qb.getWindowingSpec(dest) != null) {
            ...
            if((qbp.getAggregationExprsForClause(dest).size() != 0 || getGroupByForClause(qbp, dest).size() > 0) && qbp.getSelForClause(dest)
                    .getToken().getType() == HiveParser.TOK_SELECTDI && qbp.getWindowingExprsForClause(dest) != null) {
                if(conf.getBoolVar(HiveConf.ConfVars.HIVEMAPSIDEAGGREGATE)) {
                    if(!conf.getBoolVar(HiveConf.ConfVars.HIVEGROUPBYSKEW)) {
                        curr = genGroupByPlanMapAggrNoSkew(dest, qb, curr);
                    } else {
    ...
    private Operator genUnionPlan(String unionalias, String leftalias, Operator leftOp, String rightalias,
            ...
            if(commonTypeInfo == null) {
                throw new SemanticException(generateErrorMessage(tabref,
                        "Schema of both sides of union should " + "match: Column " + field + " is of type " + lInfo.getType()
                                .getTypeName() + " on first table " + "and " + "type " + rInfo.getType().getTypeName() + " on second table"));
            }
        ...
        if(leftOp instanceof UnionOperator || (leftOp instanceof SelectOperator && leftOp.getParentOperators() != null && !leftOp.getParentOperators()
                .isEmpty() && leftOp.getParentOperators().get(0) instanceof UnionOperator && ((SelectOperator) leftOp).isIdentitySelect())) {
        ...
        if(rightOp instanceof UnionOperator || (rightOp instanceof SelectOperator && rightOp.getParentOperators() != null && !rightOp
                .getParentOperators().isEmpty() && rightOp.getParentOperators().get(0) instanceof UnionOperator && ((SelectOperator) rightOp)
                .isIdentitySelect())) {
        ...
        Operator<? extends OperatorDesc> unionforward = OperatorFactory
                .getAndMakeChild(getOpContext(), new UnionDesc(), new RowSchema(unionoutRR.getColumnInfos()));
    ...
    private Operator<? extends OperatorDesc> genInputSelectForUnion(Operator<? extends OperatorDesc> origInputOp,
            Map<String, ColumnInfo> origInputFieldMap, String origInputAlias, RowResolver unionoutRR, String unionalias) throws SemanticException {
        ...
        while(oIter.hasNext()) {
            ...
            ExprNodeDesc column = new ExprNodeColumnDesc(oInfo.getType(), oInfo.getInternalName(), oInfo.getTabAlias(), oInfo.getIsVirtualCol(),
                    oInfo.isSkewedCol());
        ...
        Operator<SelectDesc> newInputOp = OperatorFactory
                .getAndMakeChild(new SelectDesc(columns, colName), new RowSchema(rowResolver.getColumnInfos()), columnExprMap, origInputOp);
     ...
     * @throws SemanticException
     */
    private ExprNodeDesc genSamplePredicate(TableSample ts, List<String> bucketCols, boolean useBucketCols, String alias, RowResolver rwsch,
            QBMetaData qbm, ExprNodeDesc planExpr) throws SemanticException {
        ...
        if(top == null) {
            ...
            try {
                ...
                for(int i = 0; i < fields.size(); i++) {
                    ...
                    ColumnInfo colInfo = new ColumnInfo(fields.get(i).getFieldName(),
                            TypeInfoUtils.getTypeInfoFromObjectInspector(fields.get(i).getFieldObjectInspector()), alias, false);
            ...
            for(FieldSchema part_col : tab.getPartCols()) {
                ...
                rwsch.put(alias, part_col.getName(),
                        new ColumnInfo(part_col.getName(), TypeInfoFactory.getPrimitiveTypeInfo(part_col.getType()), alias, true));
            }
                ...
                if(!unsample) {
                    ...
                    if(numBuckets > 0) {
                        ...
                        ExprNodeDesc randFunc = TypeCheckProcFactory.DefaultExprProcessor
                                .getFuncExprNodeDesc("rand", new ExprNodeConstantDesc(Integer.valueOf(460476415)));
    ...
    private void setupStats(TableScanDesc tsDesc, QBParseInfo qbp, Table tab, String alias, RowResolver rwsch) throws SemanticException {
        ...
        if((!qbp.isAnalyzeCommand() && qbp.getAnalyzeRewrite() == null) || (qbp.getAnalyzeRewrite() != null && !HiveConf
        ...
        for(String alias : qb.getSubqAliases()) {
            ...
            if(qb.getViewToTabSchema().containsKey(alias)) {
                ...
                if(operator instanceof SelectOperator) {
                    ...
                    throw new SemanticException(
                            "View " + alias + " is corresponding to " + operator.getType().name() + "," + " rather than a SelectOperator.");
                }
            }
        }
    ...
    private void rewriteRRForSubQ(String alias, Operator operator, boolean skipAmbiguityCheck) throws SemanticException {
        ...
        for(ColumnInfo colInfo : rr.getColumnInfos()) {
            ...
            if("".equals(tmp[0]) || tmp[1] == null) {
                ...
                // if it's wrapped by top-level select star query, skip ambiguity check (for backward
                // compatibility)
                tmp[1] = colInfo.getInternalName();
            }
    ...
    private Operator genLateralViewPlan(QB qb, Operator op, ASTNode lateralViewTree) throws SemanticException {
        ...
        Operator lvForward = putOpInsertMap(
                OperatorFactory.getAndMakeChild(new LateralViewForwardDesc(), new RowSchema(lvForwardRR.getColumnInfos()), op), lvForwardRR);
        ...
        Operator udtfPath = genSelectPlan(null, (ASTNode) lateralViewTree.getChild(0), blankQb, lvForward, null,
        ...
        Operator lateralViewJoin = putOpInsertMap(OperatorFactory
                .getAndMakeChild(new LateralViewJoinDesc(allColumns, outputInternalColNames), new RowSchema(lateralViewRR.getColumnInfos()), allPath,
     ...
     */
    private void LVmergeRowResolvers(RowResolver source, RowResolver dest, Map<String, ExprNodeDesc> colExprMap,
    ...
    private static void walkASTMarkTABREF(TableMask tableMask, ASTNode ast, Set<String> cteAlias, Context ctx, Hive db,
            Map<String, Table> tabNameToTabObject, Set<Integer> ignoredTokens) throws SemanticException {
        ...
        while(!queue.isEmpty()) {
            ...
            if(astNode.getToken().getType() == HiveParser.TOK_TABREF) {
                ...
                for(int index = 1; index < astNode.getChildCount(); index++) {
                    ...
                    if(ct.getToken().getType() == HiveParser.TOK_TABLEBUCKETSAMPLE || ct.getToken().getType() == HiveParser.TOK_TABLESPLITSAMPLE || ct
                            .getToken().getType() == HiveParser.TOK_TABLEPROPERTIES) {
    ...
    // unparseTranslator.
    protected static ASTNode rewriteASTWithMaskAndFilter(TableMask tableMask, ASTNode ast, TokenRewriteStream tokenRewriteStream, Context ctx,
            Hive db, Map<String, Table> tabNameToTabObject, Set<Integer> ignoredTokens) throws SemanticException {
        ...
        if(ast.getChildCount() > 0 && HiveParser.TOK_CTE == ((ASTNode) ast.getChild(0)).getToken().getType()) {
            ...
            for(int index = 1; index < ast.getChildCount(); index++) {
                walkASTMarkTABREF(tableMask, (ASTNode) ast.getChild(index), cteAlias, ctx, db, tabNameToTabObject, ignoredTokens);
            }
        }
     ...
     *  从子ASTNode继续分析
     *  -
     *  5、Resolve Parse Tree Materialization is allowed if it is not a view definition
     *  如果不是在定义视图，则进行解析树的元数据处理
     *
     * @param ast
     * @param plannerCtx
     * @return
    ...
    boolean genResolvedParseTree(ASTNode ast, PlannerContext plannerCtx) throws SemanticException {
        ...
        // 3. analyze create view command
        // TODO_MA 注释: 33333 处理create view 命令
        if(ast.getToken().getType() == HiveParser.TOK_CREATEVIEW || ast.getToken().getType() == HiveParser.TOK_CREATE_MATERIALIZED_VIEW || (ast
                .getToken().getType() == HiveParser.TOK_ALTERVIEW && ast.getChild(1).getType() == HiveParser.TOK_QUERY)) {
        ...
        // TODO_MA 注释: 44444 AST 变成 QB，其实就是将多个不能分开执行的 TOK 封装成一个 QB
        // TODO_MA 注释：之后的处理，会将 QB 变成 Operator（对应的一个关键字(sql一个语法关键字)）
        // TODO_MA 注释：之后的处理，就会将 AST 变成  OperatorTree(logical plan)
        // 4. continue analyzing from the child ASTNode.
        Phase1Ctx ctx_1 = initPhase1Ctx();
        preProcessForInsert(child, qb);
        if(!doPhase1(child, qb, ctx_1, plannerCtx)) {
            // if phase1 Result false return
            return false;
        }
        LOG.info("Completed phase 1 of Semantic Analysis");

        // TODO_MA 注释: 55555 处理元数据相关
        // TODO_MA 注释：涉及到 ： 跟 HDFS， MySQL 打交道
        // TODO_MA 注释：from student，转换成 MR ，应该要理解成： InputFormat.setInput(student表对应的HDFS目录)
        // 5. Resolve Parse Tree Materialization is allowed if it is not a view definition
     ...
     *  从解析树生成Gen OP树  OperatorTree
     *  -
     *  3、Deduce Resultset Schema（selct ...... 每个字段，我给你构造成一个 Field）
     *  推导结果集模式  CBO优化
     *  -
     *  4、Generate ParseContext for Optimizer & Physical compiler
     *  为优化器和物理编译器生成解析上下文
     *  -
     ...
     *  sql当中写了很多的无用的字段，但是呢，最终执行逻辑不需要这些字段。列裁剪
     *  -
     *  9、Optimize Physical op tree & Translate to target execution engine (MR, Spark, TEZ..)
     *  优化物理操作树并转换为目标执行引擎（MR，TEZ ..）
     *  -
     *  10、put accessed columns to readEntity
     *  将访问的列放入 ReadEntity(要读取的列的信息)
     *  -
     *  11、if desired check we're not going over partition scan limits
     *  如果需要检查，我们不会超过分区扫描限制
     *  如果你现在写了一个SQL查询，针对分区表做的，
     *  分区表可能在扫描的时候，有分区的限制，
     *  -
     *  整个这个流程的输出：  TaskTree
     *  输入的 ASTTree 后续的 QB 的生成，逻辑执行计划、逻辑执行计划的优化、物理执行计划的切分、物理执行计划的优化、
     *  以及 MapReduce 任务的生成全部都在这 1 万多行的代码里边的逻辑中。
     *  -
     *  物理执行计划和逻辑执行计划的区别： 逻辑执行计划是一个Operator图， 物理执行计划是一个Task图
     *  物理执行计划是把逻辑执行计划切分成子图
     *  物理执行计划图的每个Task结点内是一个Operator结点的子图
     *  -
     *  AST -> ParseTree -> OperatorTree -> Optimizer -> TaskTree
     *
     * @param ast
     * @param plannerCtx
     * @throws SemanticException
    ...
    void analyzeInternal(ASTNode ast, PlannerContext plannerCtx) throws SemanticException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 11111 ： 解析 抽象语法树 生成 解析树
         *  AST Tree仍然非常复杂，不够结构化，不方便直接翻译为MapReduce程序，
         *  AST Tree 转化为 QueryBlock 就是将 SQL 进一部抽象和结构化.
         *  QueryBlock是一条 SQL 最基本的组成单元，包括三个部分：输入源，计算过程，输出。
         *  简单来讲一个 QueryBlock 就是一个子查询。
         *  在这个环节中，会有和元数据库的交互。
         *  把 ASTNode 转变成由 QB 组成的树子查询
         */
        // 1. Generate Resolved Parse tree from syntax tree
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 222222 ：解析 解析树/QB 生成一个 Operator Tree， 其实就是生成了一个 DAG
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 33333： 推导结果集模式 FieldSchema 就是包装一个字段 类型信息
         *  select 后面的字段的 schema是什么
         */
        // 3. Deduce Resultset Schema
        if(createVwDesc != null && !this.ctx.isCboSucceeded()) {
            // 根据 RowResolver 来解析得到 ResultSet Schema
            resultSchema = convertRowSchemaToViewSchema(opParseCtx.get(sinkOp).getRowResolver());
        } else {
            // CBO优化的作用： 在多表进行 join 的时候，会自动的选择最优的 join 顺序
            // Hive 自0.14.0开始，加入了一项 "Cost Based Optimizer" 来对 HQL 执行计划进行优化，这个功能通过 "hive.cbo.enable" 来开启。
            // 在Hive 1.1.0之后，这个 feature 是默认开启的,它可以自动优化 HQL 中多个 JOIN 的顺序，并选择合适的 JOIN 算法
            // resultSchema will be null if
            // (1) cbo is disabled;
            // (2) or cbo is enabled with AST return path (whether succeeded or not, resultSchema will be re-initialized)
            // It will only be not null if cbo is enabled with new return path and it succeeds.
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 44444 : 构建 ParseContext, 为第七步的 优化 做准备
         */
        // 4. Generate Parse Context for Optimizer & Physical compiler
        copyInfoToQueryProperties(queryProperties);
        ParseContext pCtx = new ParseContext(queryState, opToPartPruner, opToPartList, topOps, new HashSet<JoinOperator>(joinContext.keySet()),
                new HashSet<SMBMapJoinOperator>(smbMapJoinContext.keySet()), loadTableWork, loadFileWork, columnStatsAutoGatherContexts, ctx,
                idToTableNameMap, destTableId, uCtx, listMapJoinOpsNoReducer, prunedPartitions, tabNameToTabObject, opToSamplePruner, globalLimitCtx,
                nameToSplitSample, inputs, rootTasks, opToPartToSkewedPruner, viewAliasToInput, reduceSinkOperatorsAddedByEnforceBucketingSorting,
                analyzeRewrite, tableDesc, createVwDesc, queryProperties, viewProjectToTableSchema, acidFileSinks);
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 555555 处理视图定义
         *  视图其实就是表的另外一种存在形式，表中是有真实的数据，视图只是某一个未执行的SQL的结果集的别名
         */
        // 5. Take care of view creation
        ...
        optm.setPctx(pCtx);       // pCtx = ParseContext
        optm.initialize(conf);    // 初始化的时候， 加入了很多的各种 Optimizer： 构造 List transformations
        pCtx = optm.optimize();   // 依次遍历出来所有的 Optimizer， 挨个儿执行优化
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 88888 可能的话，进行列裁剪（column pruning）
         */
        // 8. Generate column access stats if required - wait until column pruning takes place during optimization
        boolean isColumnInfoNeedForAuth = SessionState.get().isAuthorizationModeV2() && HiveConf
                .getBoolVar(conf, HiveConf.ConfVars.HIVE_AUTHORIZATION_ENABLED);
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 10101010  封装要查询的列，到entity中。 FieldSchema 封装了列的信息
         *  Row 一行，一条数据，
         */
        // 10. put accessed columns to ReadEntity
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 111111111  关于处理 分区表扫描的时候，最大的分区限制
         *  如果需要检查，我们不会超过分区扫描限制
         */
        // 11. if desired check we're not going over partition scan limits
        ...
        if(scanLimit > -1) {
            ...
            if(fTask != null) {
                ...
                if(!fTask.getWork().isNotPartitioned() && fTask.getWork().getLimit() == -1 && scanLimit < fTask.getWork().getPartDir().size()) {
                    throw new SemanticException(ErrorMsg.PARTITION_SCAN_LIMIT_EXCEEDED, "" + fTask.getWork().getPartDir().size(),
                ...
                for(Operator<?> topOp : topOps.values()) {
                    if(topOp instanceof TableScanOperator) {
                        ...
                        if(parts.getPartitions().size() > scanLimit) {
                            throw new SemanticException(ErrorMsg.PARTITION_SCAN_LIMIT_EXCEEDED, "" + parts.getPartitions().size(),
    ...
    protected void saveViewDefinition() throws SemanticException {
        ...
        // Now expand the view definition with extras such as explicit column references;
        // this expanded form is what we'll re-parse when the view is referenced later.
    ...
    static List<FieldSchema> convertRowSchemaToViewSchema(RowResolver rr) throws SemanticException {
        // TODO_MA 注释：convertRowSchemaToResultSetSchema 负责获取到 FieldSchema 信息
    ...
    public ExprNodeDesc genExprNodeDesc(ASTNode expr, RowResolver input, RowResolver outerRR, Map<ASTNode, RelNode> subqueryToRelNode,
     ...
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     * 注释： 分析create table命令。
     * 如果它是常规的create-table或 create-table-like语句，则创建 DDLWork并返回true。
     * 如果它是一个create-table-as-select，我们将获得必要的信息，例如SerDe和 Storage Format，并将其放在 QB 中，然后返回 false，
     * 这表明语义分析器的其余部分需要处理与 SerDe 和存储格式有关的 select 语句。
     *
    ...
    ASTNode analyzeCreateTable(ASTNode ast, QB qb, PlannerContext plannerCtx) throws SemanticException {
        ...
        switch(command_type) {
            ...
            case CREATE_TABLE: // REGULAR CREATE TABLE DDL
                ...
                CreateTableDesc crtTblDesc = new CreateTableDesc(dbDotTab, isExt, isTemporary, cols, partCols, bucketCols, sortCols, numBuckets,
                        rowFormatParams.fieldDelim, rowFormatParams.fieldEscape, rowFormatParams.collItemDelim, rowFormatParams.mapKeyDelim,
                        rowFormatParams.lineDelim, comment, storageFormat.getInputFormat(), storageFormat.getOutputFormat(), location,
                        storageFormat.getSerde(), storageFormat.getStorageHandler(), storageFormat.getSerdeProps(), tblProps, ifNotExists,
                        skewedColNames, skewedValues, primaryKeys, foreignKeys);
            ...
            case CTLT: // create table like <tbl_name>
                ...
                if(isTemporary) {
                    ...
                    if(likeTable != null && likeTable.getPartCols().size() > 0) {
                        throw new SemanticException(
                                "Partition columns are not supported on temporary tables " + "and source" + " table in CREATE TABLE LIKE is partitioned.");
                    }
                }
                CreateTableLikeDesc crtTblLikeDesc = new CreateTableLikeDesc(dbDotTab, isExt, isTemporary, storageFormat.getInputFormat(),
            ...
            case CTAS: // create table as select
                ...
                tableDesc = new CreateTableDesc(qualifiedTabName[0], dbDotTab, isExt, isTemporary, cols, partCols, bucketCols, sortCols, numBuckets,
                        rowFormatParams.fieldDelim, rowFormatParams.fieldEscape, rowFormatParams.collItemDelim, rowFormatParams.mapKeyDelim,
                        rowFormatParams.lineDelim, comment, storageFormat.getInputFormat(), storageFormat.getOutputFormat(), location,
                        storageFormat.getSerde(), storageFormat.getStorageHandler(), storageFormat.getSerdeProps(), tblProps, ifNotExists,
    ...
    protected ASTNode analyzeCreateView(ASTNode ast, QB qb, PlannerContext plannerCtx) throws SemanticException {
        ...
        if(isMaterialized) {
            createVwDesc = new CreateViewDesc(dbDotTable, cols, comment, tblProps, partColNames, ifNotExists, orReplace, rewriteEnabled,
                    isAlterViewAs, storageFormat.getInputFormat(), storageFormat.getOutputFormat(), location, storageFormat.getSerde(),
            ...
            createVwDesc = new CreateViewDesc(dbDotTable, cols, comment, tblProps, partColNames, ifNotExists, orReplace, isAlterViewAs,
                    storageFormat.getInputFormat(), storageFormat.getOutputFormat(), storageFormat.getSerde());
        ...
        try {
            ...
            if(createVwDesc.getOrReplace() && oldView != null) {
                ...
                if(!createVwDesc.isMaterialized()) {
                    ...
                    try {
                        if((createVwDesc.getPartCols() == null || createVwDesc.getPartCols().isEmpty() || !createVwDesc.getPartCols()
                                .equals(oldView.getPartCols())) && !oldView.getPartCols().isEmpty() && !db.getPartitions(oldView).isEmpty()) {
                    ...
                    for(int child_pos = 0; child_pos < groupbyNode.getChildCount(); ++child_pos) {
                        ...
                        if(node.getToken().getType() == HiveParser.Number) {
                            if(isGbyByPos) {
                                ...
                                warn("Using constant number  " + node
                                        .getText() + " in group by. If you try to " + "use position " + "alias when hive.groupby.position.alias is false, the " + "position alias will be ignored.");
                            }
                        }
                    }
                }
                            ...
                            if(isObyByPos) {
                                if(!isAllCol) {
                                    ...
                                    if(pos > 0 && pos <= selectExpCnt) {
                                        ...
                                        throw new SemanticException(ErrorMsg.INVALID_POSITION_ALIAS_IN_ORDERBY
                                                .getMsg("Position " + "alias: " + pos + " does not exist\n" + "The Select List is indexed from 1 to " + selectExpCnt));
                                    }
                                } else {
                                ...
                                warn("Using constant number " + node
                                        .getText() + " in order by. If you try to " + "use position alias" + " when hive.orderby.position.alias is false, the " + "position alias will be ignored.");
                            }
                        }
                    }
                }
     ...
     * @throws SemanticException
     */
    void buildPTFReduceSinkDetails(PartitionedTableFunctionDef tabDef, RowResolver inputRR, ArrayList<ExprNodeDesc> partCols,
            ArrayList<ExprNodeDesc> orderCols, StringBuilder orderString, StringBuilder nullOrderString) throws SemanticException {
    ...
    Operator genWindowingPlan(QB qb, WindowingSpec wSpec, Operator input) throws SemanticException {
        ...
        if(!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
            ...
            for(WindowExpressionSpec wExprSpec : wSpec.getWindowExpressions()) {
                // Special handling of grouping function
                wExprSpec.setExpression(rewriteGroupingFunctionAST(getGroupByForClause(qb.getParseInfo(), selClauseName), wExprSpec.getExpression(),
    ...
    public static ArrayList<WindowExpressionSpec> parseSelect(String selectExprStr) throws SemanticException {
        ...
        for(int i = 0; i < childCount; i++) {
            ...
            ASTNode aliasNode = selExpr.getChildCount() > 1 && selExpr.getChild(1).getType() == HiveParser.Identifier ? (ASTNode) selExpr
                    .getChild(1) : null;
            String alias = null;
            if(aliasNode != null) {
