...
public class MapRedTask extends ExecDriver implements Serializable {
    ...
    public int execute(DriverContext driverContext) {
        ...
        try {
            ...
            if (!ctx.isLocalOnlyExecutionMode() && conf.getBoolVar(HiveConf.ConfVars.LOCALMODEAUTO)) {
                ...
                totalInputFileSize = Utilities.getTotalInputFileSize(inputSummary, work.getMapWork(),
                        samplePercentage);
                totalInputNumFiles = Utilities.getTotalInputNumFiles(inputSummary, work.getMapWork(),
                        samplePercentage);
                ...
                /**
                 * TODO 根据规则计算是否可以进行本地运算
                 */
                String reason = MapRedTask.isEligibleForLocalMode(conf, numReducers, totalInputFileSize,
                        totalInputNumFiles);
            ...
            if (!runningViaChild) {
                ...
                /**
                 * TODO_c 最终还是执行 ExecDriver 中的 execute 方法
                 */
            ...
            /**
             * 这个地方会告诉我们，如果环境变量没有配置 HADOOP_HOME 变量，程序就会报错
             */
            // enable assertion
            ...
            String cmdLine = hadoopExec + " jar " + jarCmd + " -plan " + planPath.toString() + " " + isSilent +
                    " " + hiveConfArgs;
            ...
            StreamPrinter outPrinter = new StreamPrinter(executor.getInputStream(), null,
    ...
    static void configureDebugVariablesForChildJVM(Map<String, String> environmentVariables) {
        ...
        assert hadoopClientOpts.contains(environmentVariables.get(HIVE_MAIN_CLIENT_DEBUG_OPTS)) :
                HADOOP_CLIENT_OPTS + " environment variable must contain debugging parameters, when JVM in " +
                        "debugging mode";
        ...
        if (environmentVariables.get(HIVE_DEBUG_RECURSIVE).equals("y")) {
            ...
            environmentVariables.put(HADOOP_OPTS_KEY,
                    environmentVariables.get(HADOOP_OPTS_KEY).replace(environmentVariables.get(HADOOP_CLIENT_OPTS), ""));
            ...
            String newHadoopClientOpts =
                    hadoopClientOpts.replace(environmentVariables.get(HIVE_MAIN_CLIENT_DEBUG_OPTS),
            ...
            String newHadoopClientOpts =
                    hadoopClientOpts.replace(environmentVariables.get(HIVE_MAIN_CLIENT_DEBUG_OPTS), "").trim();
